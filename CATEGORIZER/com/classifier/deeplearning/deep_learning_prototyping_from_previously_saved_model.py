...
#        print('Adding sentences : '+str(counter))
   ...
   # model.most_similar("terrible")
    ...
    forest = forest.fit( trainDataVecs, train["output"] )

    print "Forest has been trained, we now make the prediction"

    # Test & extract results
    result = forest.predict( testDataVecs )

    # Write the test results
    output = pd.DataFrame( data={"Id_Product":test["id"], "Id_Categorie":result} )
    output.to_csv( "/home/sduprey/My_Data/My_OutgoingData/Word2Vec_AverageVectors.csv", index=False, quoting=3 )
