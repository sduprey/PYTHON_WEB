...
#                    print("Be cautious")
    ...
    def train_predict(self):
        ...
        for rowNumber, indices, rowDatas, label in self.data(self.train, self.label):
            ...
            self.update_w(w, n, indices, rowDatas, prediction, good_answer)
            # print out progress, so that we know everything is working
            if rowNumber % 100 == 0 & rowNumber !=  0 :
                print('%s encountered: %d current logloss: %f' % (
                    datetime.now(), rowNumber, (loss/33.)/rowNumber))
...
if __name__ == "__main__":
    nb_categories = 5789
    K = [k for k in range(nb_categories)]
    for category_number in K :
        best_online = my_online_learner_single_category(train="/home/sduprey/My_Data/My_Cdiscount_Challenge/full_hashed_training_matrix.csv",label = '/home/sduprey/My_Data/My_Cdiscount_Challenge/hashed_data_training_output_vector.csv',test="/home/sduprey/My_Data/My_Cdiscount_Challenge/full_hashed_testing_matrix.csv", D=1048576 , alpha= .1, output_file="/home/sduprey/My_Data/My_Cdiscount_Challenge/submission.csv", category_number=category_number)
        # getting the weighted model for our category
        w = best_online.train_predict()
        filename_category_model = "/home/sduprey/My_Data/My_Cdiscount_Challenge/submission"+"_"+str(category_number)+".csv"
