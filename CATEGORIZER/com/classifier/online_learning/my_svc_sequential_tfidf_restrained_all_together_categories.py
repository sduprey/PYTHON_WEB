...
from svc_classifier_full_hashed import svc_classifier_full_hashed

class my_svc_classifier_all_together_category:
    ...
    def train_predict_all_labels(self,Xtrain, ytrain,Xtest):
        ...
        svc_clf=svc_classifier_full_hashed(nb_categories=self.nb_categories) 
        X_test_xgb_pred = svc_clf.train_predict_all_labels(Xtrain, ytrain,Xtest)
...
#    D=1048576
    ...
    svc_all_categories = my_svc_classifier_all_together_category(D=1048576, nb_categories=5789)
    ...
    Xtrain = svc_all_categories.load_sparse_csr(my_saving_training_matrix_file)
    ...
    ytrain = svc_all_categories.load_csv_output_vector(my_saving_training_outputvector_file_path)
...
#        print('Category size : '+str(sum(y_i)))
    ...
    Xtest = svc_all_categories.load_sparse_csr(my_saving_testing_matrix_file)
    print('Testing sample size')
    print(Xtest.shape[0])
    print(Xtest.shape[1])
      
    # getting the weighted model for our category
    print('Training and predicting svc classifier for each category')
    my_svc_predictions = svc_all_categories.train_predict_all_labels(Xtrain, ytrain, Xtest)
    
    filename_category_model = "/home/sduprey/My_Data/My_Cdiscount_Challenge/svc_sequential_whole_tfidf_restrained_data_all_together_submission.csv"
    print("Saving model to our file : " + filename_category_model)
    svc_all_categories.save_model(filename_category_model, my_svc_predictions)
